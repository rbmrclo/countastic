...
(function () {
  ...
  Countastic.prototype = {
    ...
    _settings: {
      ...
      strictMax:            false,            // If true, won't be able to type anymore after reaching maxCount
      ...
      delimiter:            ',',              // Separator
      highlighter:          'red',            // #f00 #ff0000 red rgb(255, 0, 0)
      onAfterMax:           function(){},     // Do whatever you like after maxCount
      onWarning:            function(){},     // Do whatever you like on warning
      onBeforeMax:          function(){},     // Do whatever you like before maxCount
    ...
    _init: function (options) {
      ...
      if (!countable) {
        throw new Error('missing parameters: countable');
      }
    ...
    attachEventHandlers: function (counter, countable) {
      var eventHandlers = ['keyup', 'paste', 'remove', 'blur'];

      for (var i = 0; i < eventHandlers.length; i++){
        countable.addEventListener(eventHandlers[i], ( function () { this.countCheck(counter, countable) }).bind(this), false);
      }
    },

    countCheck: function (counter, countable) {
    ...
    setCssAndCallbacks: function (count, settings) {
      ...
      if (!counter.classList.contains(settings.successClass) && !counter.classList.contains(settings.alertClass) && !counter.classList.contains(settings.warningClass)) {
        ...
        settings.onAfterMax(this.countInt(revCount, count), this, counter);
        ...
        settings.onBeforeMax(this.countInt(revCount, count), this, counter);
      }
    }
  }
